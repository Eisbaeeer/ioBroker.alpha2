...
adapter.on('stateChange', function (id, state) {
    ...
    if (state && !state.ack) {
		...
		var heatarea = '1';
			...
			// Post DATA to DEVICE
			var data = '<?xml version="1.0" encoding="UTF-8"?> <Devices> <Device> <ID>'+ adapter.namespace.DEVICE.ID.val +'</ID> <HEATAREA nr="'+ heatarea +'"> <T_TARGET>'+ id.val +'</T_TARGET> </HEATAREA> </Device> </Devices>';
			httpPost(data);
	}
});
